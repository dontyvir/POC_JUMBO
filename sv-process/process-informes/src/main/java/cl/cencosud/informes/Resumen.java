...
public class Resumen {
    ...
    * @throws SQLException
    */
   public void generar(Connection con) throws SQLException {
      ...
      gregDesde.setFirstDayOfWeek(Calendar.MONDAY);
	      gregDesde.set(AGNO, MES_INICIO-1, DIA_INICIO);
	      desde = new java.sql.Date(gregDesde.getTimeInMillis());
       
	} else {//Crontab
		conf = Db.getConfiguracionSemanal(con);
	      if (conf.getUltimaFechaSemanal() == null) {
	         GregorianCalendar fecha = new GregorianCalendar(2008, 0, 7); //primer lunes del año
	         conf.setUltimaFechaSemanal(fecha.getTime());
	         Db.addConfiguracionSemanal(con, conf);
	      }
   		desde = new java.sql.Date(conf.getUltimaFechaSemanal().getTime());
 	}
