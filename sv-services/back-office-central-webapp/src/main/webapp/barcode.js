// This material is copyrighted material of RenderBar Imaging?
// Distributing, viewing, modifying, or otherwise using this
...
Barcode.prototype.shiftChar         = getShift;
...
function getHTML(){
  ...
  var gif       = "";
  var htm       = "";
  if (isNaN(this.height)) h = 25;
  if (isNaN(this.width)) w = 2;
  if (this.height < 5) this.height = 25;
  ...
  for (var i = 0; i < encoded.length; i++){ 
    ...
    htm += "<img src\=\"" +img_dir+ gif + "\" style=\"height:'" + this.height + "'\; width:'" + (this.width * chrCount) + "'\;\">";
  }
  if (readable){
    if (type == code128){
      while (value.indexOf(char(this.A)) != -1)     input = input.replace(char(this.A), "");
      while (value.indexOf(char(this.B)) != -1)     input = input.replace(char(this.B), "");
      while (value.indexOf(char(this.C)) != -1)     input = input.replace(char(this.C), "");
      while (value.indexOf(char(this.Shift)) != -1) input = input.replace(char(this.Shift), "");
    }
    htm += "<br><div style=\"display:'inline'; text-align:'center';\">" + value + "</div>";
  }
  html = "<div style=\"display:'inline'; text-align:'center';\">" + htm + "</div>";
  if (this.parentEl != null) this.parentEl.innerHTML = html;
}
...
function Code128(input){
  ...
  if (curCodeTable == this.C) input += ((input.length % 2 != 0) ? "0" : "");
  for (var i = 0; i < input.length; i++){
    chr = input.charCodeAt(i);
    if (chr == 199 || chr == 200 || chr == 201){
      switch (curCodeTable){
        case this.codeA: if (chr == 199){curCodeTable = this.C;}else{if (chr == 200) curCodeTable = this.B;}  break;
        case this.codeB: if (chr == 199){curCodeTable = this.C;}else{if (chr == 201) curCodeTable = this.A;}  break;
        case this.codeC: if (chr == 200){curCodeTable = this.B;}else{if (chr == 201) curCodeTable = this.A;}  break;
      }
    }
    if (curCodeTable == this.A || curCodeTable == this.B){
      switch (true){
...
function EAN13(input){
  ...
  if (input.length == 12){
    ...
    this.checkDigit = 10 - ((evenSum + (oddSum * 3)) % 10)
    this.checkDigit = (this.checkDigit == 10 ? 1 : this.checkDigit);
  ...
  if (compress){
    ...
    switch (true){
      ...
      case expFourth.test(input): expFourth    = new RegExp("(^[0-9]{6})([0-9]{4})([0-9]{1}$)");
                                  ...
                                  break;
      default                   : autoCompress = false; break;
    }
  }
  if (compress){
    input  += suppliment;
