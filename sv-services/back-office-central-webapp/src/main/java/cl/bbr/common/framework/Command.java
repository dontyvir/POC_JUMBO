...
public abstract class Command extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	protected String cmd_dsp_error	= "";
	 ...
	 protected final void doGet(HttpServletRequest req, HttpServletResponse res) 
		...
		// Si "PagErr" es 1, muestra pagina con MENU, y el mensaje de error en medio.
		// Si "PagErr" es 2, se muestra sólo el mensaje de error
		String PagErr = (req.getParameter("PagErr")==null?"1":req.getParameter("PagErr"));
		...
		if ( session.getAttribute("user") != null ){
			...
			logger.info("User: (" + usr.getId_usuario() + ") " + usr.getLogin());
		...
		try {
			isAutorizado = VerifyAccessControl(usr, logger);
		} catch (SystemException e2){
			logger.error("SystemException: " + e2.getMessage());
			e2.printStackTrace();
		...
		if(isAutorizado){
			...
			String url = "";
			res.sendRedirect( cmd_dsp_error +"?mensaje="+msg+"&url="+url+"&PagErr="+PagErr);
			return;
		}
	...
	private boolean VerifyAccessControl(  UserDTO usr , Logging logger ) throws SystemException{
		ComandoDTO 	cmd 		= null;
		...
		try {
			...
			throw new SystemException("Error al solicitar BizDelegate");
		}
