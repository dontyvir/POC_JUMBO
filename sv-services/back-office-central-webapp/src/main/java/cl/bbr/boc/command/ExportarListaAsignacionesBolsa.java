...
public class ExportarListaAsignacionesBolsa extends Command {

    protected void Execute(HttpServletRequest req, HttpServletResponse res,
            UserDTO usr) throws Exception {

        logger.debug( "Inicio ExportarListaAsignacionesBolsa" );
        try {
            ...
            File archivoNuevo = File.createTempFile("asignacionesdebolsa", ".xls", directorioDestino);
            ...
            res.setHeader("Content-Disposition", "attachment;filename=Asignaciones_bolsas_al_" + Utils.getFechaActualByPatron("dd-MM-yyyy_HH-mm")  + ".xls");
        ...
        logger.debug( "Fin ExportarListaAsignacionesBolsa" );
    }
    ...
    private HSSFWorkbook planillaExcel() throws BocException {
        ...
        celda.setCellValue(new HSSFRichTextString("RUT CLIENTE"));        
        celda = fila.createCell(1);
        celda.setCellStyle(estiloCelda);
        celda.setCellValue(new HSSFRichTextString("CÓDIGO BOLSA"));
        
        hoja1.setColumnWidth(0, 256*15 );
        hoja1.setColumnWidth(1, 256*12 );
                
        List asignaciones = biz.getAsignacionesBolsas();
        ...
        for (int i = 0; i < asignaciones.size(); i++) {           
        	Hashtable asign = (Hashtable) asignaciones.get(i);
            fila = hoja1.createRow((short) (i+1));
            
            celda = fila.createCell(0);
            celda.setCellStyle(estiloCeldaNormal);
            celda.setCellValue(new HSSFRichTextString((String)asign.get("rut_cliente")));
            celda = fila.createCell(1);
            celda.setCellStyle(estiloCeldaNormal);
            celda.setCellValue(new HSSFRichTextString((String)asign.get("cod_bolsa")));
