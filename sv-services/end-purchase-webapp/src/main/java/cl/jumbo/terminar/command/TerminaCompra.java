package cl.jumbo.terminar.command;
...
import cl.jumbo.terminar.utils.Formatos;
import cl.jumbo.terminar.exceptions.FuncionalException;

public class TerminaCompra extends TimerTask {
	...
	public void run() {		
        ...
        t.terminaComprasIncompletas(new BizDelegate(),rb);
		logger.info("*** FIN ***");		
	}
...
//                logger.debug("Pedido CAT ("+j+") "+ped.getId_pedido());
    ...
    private void actualizarCompraWeb(BizDelegate biz, String valor){
    	 try {
 			biz.actualizaParametroByName("TERMINA_COMPRA_WEB", valor);
 		} catch (FuncionalException e) {		
 			 logger.error("Error Actualizar (TERMINA_COMPRA_WEB), " + e);
 		}	
    }
