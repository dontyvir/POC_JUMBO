...
function cargalo() {
	...
	var direccion = '<%=request.getContextPath() %>/OrderComplete?tipoAutorizacion=<%=tipoAutorizacion%>&numeroTarjeta=<%=numeroTarjeta%>&rutCliente=<%=rutCliente%>&usoClave=<%=usoClave%>&glosaRespuesta=<%=glosaRespuesta%>&codigoRespuesta=<%=codigoRespuesta%>';
	//alert(direccion)
