...
public class EnviarCorreoExcel extends HttpServlet implements Servlet {

//20112012 VMatheu
	protected Logging logger = new Logging(this);

	public EnviarCorreoExcel() {
		super();
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            if (solicitud.equals("enviarExcel")) {
                HSSFWorkbook objWB = planillaExcel(req);
                String rutaUpload = rb.getString("conf.dir.html.excel")+ "/tmp_listas/";
                ...
                FileInputStream archivoIn = new FileInputStream(archivoNuevo);
                FileOutputStream archivoOut = new FileOutputStream(archivoNuevo);
                objWB.write(archivoOut);

                int longitud = archivoIn.available();
                byte[] datos = new byte[longitud];
                archivoIn.read(datos);
                ...
                res.setDateHeader("Expires", 0); //prevents caching at the
                res.setHeader("Cache-Control", "no-cache"); //HTTP 1.1
                res.setHeader("Cache-Control", "max-age=0");
                res.setContentType("application/vnd.ms-excel; charset=windows-1254");
                res.setContentLength(longitud);
                
                res.getOutputStream().write(datos);
...
//--20112012 VMatheu	
	 ...
	 * @throws IOException
	 */
	private HSSFWorkbook planillaExcel(HttpServletRequest req)
			throws IOException {
		...
		hoja1
				.addMergedRegion(new Region(nroFila, (short) 0, nroFila,
						(short) 4)); //FILA-INI: ;COL-INI; FILA-FIN: ;COL-FIN
		celda.setCellValue(new HSSFRichTextString(""));

		//creamos una fila para el logo
		// D:\IBM\workspaces\WORKSPACE_OTROS_DESARROLLOS\FO\WebContent\mail
		//String imagen = rb.getString("conf.dir.html") +
		// "/mail/jumbo_movil.jpg";
		...
		try {
			if (req.getParameter("tupla") != null) {
				...
				List datos = biz.clientesGetUltComprasCategoriasProductos(
						tupla, req.getParameter("ses_loc_id").toString(), Long
								.parseLong(req.getParameter("ses_cli_id")
										.toString()), Long.parseLong(req
								.getParameter("ses_cli_rut").toString()));

				for (int j = 0; j < datos.size(); j++) {
					UltimasComprasCategoriasDTO cat = (UltimasComprasCategoriasDTO) datos
							.get(j);

					//Escribimos la categoría
					HSSFFont fuenteCategoria = estiloFuente(objWB,
							HSSFFont.BOLDWEIGHT_BOLD, (short) 53); //8 es negro
					HSSFCellStyle estiloCeldaCategoria = estiloCelda(objWB,
							fuenteCategoria, HSSFCellStyle.ALIGN_CENTER);

					fila = hoja1.createRow((short) (nroFila));

					celda = fila.createCell((short) 0);
					celda.setCellStyle(estiloCeldaCategoria);
					celda.setCellValue(new HSSFRichTextString(cat
							.getCategoria()));

					hoja1.addMergedRegion(new Region(nroFila, (short) 0,
							nroFila, (short) 4)); //FILA-INI: ;COL-INI;
					// FILA-FIN: ;COL-FIN
					nroFila++;
					...
					for (int i = 0; i < prod.size(); i++) {
						...
						HSSFFont fuenteNormal = estiloFuente(objWB,
								HSSFFont.BOLDWEIGHT_NORMAL, (short) 8);
						HSSFCellStyle estiloCeldaNormalLeft = estiloCelda(
								objWB, fuenteNormal, HSSFCellStyle.ALIGN_LEFT);
						HSSFCellStyle estiloCeldaNormalRight = estiloCelda(
								objWB, fuenteNormal, HSSFCellStyle.ALIGN_RIGHT);

						fila = hoja1.createRow((short) (nroFila));

						//cambia la cantidad de manera que sea acorde con el
						// intervalo al momento del despliegue
						double mod = producto.getCantidad()
								% producto.getInter_valor();
						if (mod > 0) {
							double div = producto.getCantidad() - mod
									+ producto.getInter_valor();
						...
						// HSSFRichTextString(Formatos.formatoIntervalo(producto.getCantidad()).replace('.',',')));
						...
						celda.setCellType(HSSFCell.CELL_TYPE_STRING);
						if (producto.getEsParticionable().equalsIgnoreCase("S")) {
							celda.setCellValue(new HSSFRichTextString(" 1/"
									+ producto.getParticion() + " "
									+ producto.getTipre()));
						} else {
							celda.setCellValue(new HSSFRichTextString(producto
