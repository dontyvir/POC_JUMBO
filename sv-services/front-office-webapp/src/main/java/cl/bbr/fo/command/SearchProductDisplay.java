 ...
 * 
 */
public class SearchProductDisplay extends Command {
	...
	protected void execute(HttpServletRequest arg0, HttpServletResponse arg1)
		...
		try {
			...
			// Recuperar datos de la categoría a buscar el patrón
			long idcategoria = 0;
			...
			for( int i = 0; i < list_categoria.size() ; i++ ) {
				...
				if ( idcategoria == data.getId()){
					fila.setVariable("{class_menu}",  "menuSeleccionado");
					fila.setVariable("{menu_cerrado}",  "");
				}else{
					fila.setVariable("{menu_cerrado}",  "display:none;");
					if ((i+1) % 2 == 0){
			...
			if ( arg0.getParameter("cai") != null  ) {
				...
				if (cat.getBanner() != null && cat.getBanner().compareTo("") != 0) {
					IValueSet valueset_show_banner = new ValueSet();
					List lista_show_banner = new ArrayList();
					valueset_show_banner.setVariable("{banner}", cat.getBanner()+"");
					lista_show_banner.add(valueset_show_banner);
					top.setDynamicValueSets("SHOW_BANNER",lista_show_banner);
				}
				...
				List list_prod = biz.getSearchListSeccion( session.getAttribute("ses_loc_id").toString(), idcategoria, cliente_id, "", "", list_patrones, l_torec );
