...
function showOlvidaClaveLayer() {
	...
	layerOlvidaClave.style.width = '452px';
	layerOlvidaClave.style.height = '200px';
...
function logonInDespacho(type_lbox) {
...
function logon3(type_lbox) {
	...
	updateComunaBeforeLogon();
	//actualizaCarro();
	irACategoryDisplay(f2.cli_rut.value, f2.clave.value, type_lbox, f2.url_to.value);
}
function updateComunaBeforeLogon() {

	$j.post("/FO/ChangeComunaSession", {
		id_comuna : '4'
	}, function(datos) {
...
function irACategoryDisplay(rut_x, clave_x, type_x, url_x) {
      ...
      jQuery(xml).find('login').each(function() {
        ...
        if ( mensaje == 'OK' ) {
          ...
          if(isCatalogoExt == "true"){
    		  destination = '/FO/CategoryDisplays';
          }else{
        	  destination = '/FO/CategoryDisplay';
          }  
...
function dejarFoco() {
	if(!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test( jQuery('#email').val() )) { 
		jQuery('#email').val('Ingresa tu mail');
		//return;
	} 
