...
//      set_total_desc( $j('#total_desc_carro_compras').val( , $j('#promo_desc_carro_compras').value , $j('total_desc_carro_compras_sf').value ); 
    }
  };
...
function obtenerProductosCarro(){
	...
	for (var x = 0; x < v_inputs.length; x++){
		if (v_inputs[x].type == "hidden"){
			prod = v_inputs[x].id;
			if (isNumeric(prod.substr(5,prod.length))) {
		...
		var requestOptions 	= {
			...
			'onSuccess': function (REQUEST) {
				...
				if (document.getElementById("respuestaValidacionCarroCompras")){
					if (document.getElementById("respuestaValidacionCarroCompras").innerHTML == ""){
						//$j("#respuestaValidacionCarroCompras").html("");
						//$j("#respuestaValidacionCarroCompras").html(respuesta);
						document.getElementById("respuestaValidacionCarroCompras").innerHTML = respuesta;
					}
				}else 
				if (document.getElementById("respuestaRescateComprasPaso1")){
					if(document.getElementById("respuestaRescateComprasPaso1").innerHTML == ""){
				...
				if (document.getElementById("respuestaRescateComprasPaso1b")){
					if (document.getElementById("respuestaRescateComprasPaso1b").innerHTML == ""){				
						document.getElementById("respuestaRescateComprasPaso1b").innerHTML = respuesta;
					}
				}
			}
		};
