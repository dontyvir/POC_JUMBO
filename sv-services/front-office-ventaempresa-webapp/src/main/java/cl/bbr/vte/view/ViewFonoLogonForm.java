...
/**
 * <p>Comando que despliega el formulario de conexión a los usuarios Fono Empresa.</p>
 * 
 ...
 *  
 */
public class ViewFonoLogonForm extends Command {

	/**
	 * Despliega el formulario de conexión a los usuarios Fono Empresa.
	 * 
	 * @param arg0
	...
	protected void execute(HttpServletRequest arg0, HttpServletResponse arg1) throws Exception {
		...
		top.setVariable("{bt_inicio}", "bt-iniciocontactotelefono2.swf");
		if (session.getAttribute("bt_inicio") != null){
		    session.removeAttribute("bt_inicio");
		}
		session.setAttribute("bt_inicio", "bt-iniciocontactotelefono2.swf");
		...
		if( arg0.getParameter("rut2") != null )
			rut2 = arg0.getParameter("rut2").toString(); 
		if( arg0.getParameter("dv2") != null )
			dv2 = arg0.getParameter("dv2").toString();
		
		top.setVariable("{rut}", rut);			
		top.setVariable("{dv}", dv);		
		top.setVariable("{rut2}", rut2);			
		top.setVariable("{dv2}", dv2);		
